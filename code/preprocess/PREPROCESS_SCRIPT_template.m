    ...
    for file = 1:length(dat) %% for each file in the directory 
      ...
      %%% Make a easy TP Table for this file 
        ...
        for r = 1:length(cc) %22 
            ...
            ct = cc(r); 
            ttpRow = tptab(ct.TRIAL.TP_ROW,:); %% tp table data for that trial 
            instruct = ttpRow.png; % id if it's an instruction trial - skip and do not count towards trials
            ...
            if instruct == 0
                %%% target info 
                ttarg = ttpRow.target; % target shown
                ...
                %%% protocol info 
                tcontFB = ttpRow.cont_fb; % 
                tendptFB = ttpRow.endpt_fb;
                tisRot = ttpRow.is_rot;
                trotDeg = ttpRow.deg;
                rotRad = deg2rad(trotDeg);
                tgoDelay = ttpRow.go_delay;
                tphase = ttpRow.phase;
                ...
                terr = max(ct.ACH0); % error is an analog output 
...
writetable(df,'/Users/hannahillman/Desktop/research/MWM_project/data/preprocessed_vmr/vmr.csv')
